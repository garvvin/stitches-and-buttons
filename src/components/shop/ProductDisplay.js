...
export default function ProductDisplay({ productData, extraClass }) {
	return (
		<Link href={`/store/${productData.id}`}>
			<div
				className={`w-35 h-52 md:w-40 md:h-60 lg:w-50 lg:h-75 rounded-xl overflow-hidden bg-stone-200 flex flex-col items-center justify-center duration-300 cursor-pointer select-none border-2 border-gray-300 hover:shadow-xl ${extraClass}`}
			>
				<Image
					...
					width={200}
					height={300}
					className="h-2/3 w-4/5 bg-blue-300 object-cover outline-2 outline-stone-300"
					draggable={false}
				/>
